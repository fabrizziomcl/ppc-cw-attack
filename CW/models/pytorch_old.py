...
class PyTorchModel(DifferentiableModel):
    ...
    def __init__(
        ...
        if hasattr(model, 'training') and model.training:
            warnings.warn(
                'The PyTorch model is in training mode. Call eval() for deterministic behavior.')

    def _old_pytorch(self):
        import torch
        version = torch.__version__.split('.')[:2]
        return int(version[0]) == 0 and int(version[1]) < 4
    ...
    def batch_predictions(self, images):
        ...
        n = len(images)
        images = torch.from_numpy(images.copy()).to(self.device) # .copy() para evitar warnings
    ...
    def predictions_and_gradient(self, image, label):
        ...
        images = torch.from_numpy(image[np.newaxis].copy()).to(self.device)
    ...
    def backward(self, gradient, image):
        ...
        gradient_tensor = torch.from_numpy(gradient).to(self.device)
        
        loss = torch.dot(predictions, gradient_tensor)
        loss.backward()
