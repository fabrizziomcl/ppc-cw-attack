...
def find_kp_point(image_np, fmodel):
    ...
    dominant = np.argmax(original_logits)

    row_vectors = get_row_vectors(image_np)
    try:
        mean_vec, pcs = compute_principal_components(row_vectors)
    except np.linalg.LinAlgError:
        return None, None

    max_k = row_vectors.shape[0]
    for k in range(max_k, 0, -1):
        recon = project_and_reconstruct(row_vectors, k, mean_vec, pcs)
        recon_img = recon.reshape(h, w, c)
        logits_k = fmodel.predictions(recon_img)
        new_pred = np.argmax(logits_k)
        if new_pred != dominant:
            probs = softmax(logits_k)
            return k, probs[new_pred]
    return None, None
